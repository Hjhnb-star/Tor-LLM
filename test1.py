...
class Neo4jCVEAnalyzer:
            ...
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    if related_cves:  # If we have CWE-related CVEs
                        # Phase 1: Keyword search within CWE-related CVEs
                        if product_keywords:
                            keyword_results = self.search_cve_by_keywords(
                                product_keywords)
                            # Filter to only include CWE-related CVEs
                            keyword_results = [cve for cve in keyword_results
                                               if
                                               cve["cve_id"] in [r["cve_id"] for
                                                                 r in
                                                                 related_cves]]

                        # Phase 2: Semantic search within keyword results if any, otherwise within CWE-related CVEs
                        search_scope = [cve["cve_id"] for cve in
                                        keyword_results] if keyword_results else [
                            cve["cve_id"] for cve in related_cves]
                        semantic_results = self.search_cve_by_semantics(
                            cve_keywords,
                            search_scope
                        )

                        # If no semantic results in limited scope, try semantic search in all CWE-related CVEs
                        if not semantic_results:
                            semantic_results = self.search_cve_by_semantics(
                                cve_keywords,
                                [cve["cve_id"] for cve in related_cves]
                            )
                    else:  # No CWE matches, do global search
                        # Phase 1: Global keyword search
                        if product_keywords:
                            keyword_results = self.search_cve_by_keywords(
                                product_keywords)

                        # Phase 2: Semantic search within keyword results if any, otherwise global
                        search_scope = [cve["cve_id"] for cve in
    ...
    def test_search_with_verbose_output(self, product_keywords=None,
                                        cwe_keywords=None, cve_keywords=None):
        """带完整输入输出展示的测试"""

        # 如果没有提供参数，则使用默认测试案例
        if product_keywords is None:
            product_keywords = ["nas_sharing.cgi"]
        if cwe_keywords is None:
            cwe_keywords = "The product constructs all or part of a command using externally-influenced input from an upstream component, but it does not neutralize or incorrectly neutralizes special elements that could modify the intended command when it is sent to a downstream component."
        if cve_keywords is None:
            cve_keywords = "Command injection vulnerability in nas_sharing.cgi script due to insufficient input validation in the 'system' parameter, allowing attackers to execute arbitrary commands."
        ...
        print(f"CWE关键词: {cwe_keywords}")
        print(f"CVE语义查询: {cve_keywords}")

        # 2. 执行CWE搜索
        print(f"\n[阶段1] CWE搜索 - 查询: '{cwe_keywords}'")
        cwe_results = self.search_cwe(cwe_keywords)
        print(f"找到 {len(cwe_results)} 个相关CWE:")
        for cwe in cwe_results[:3]:  # 最多显示3个
            print(
                f"- {cwe['cwe_id']}: {cwe['cwe_name']} (相似度: {cwe['similarity']:.3f})")
            print(f"  描述: {cwe['description'][:100]}...")

        # 3. 查找相关CVE
        cwe_ids = [r["cwe_id"] for r in cwe_results]
        print(f"\n[阶段2] 查找CWE相关的CVE - CWE IDs: {cwe_ids}")
        related_cves = self._find_cves_by_cwe_ids(cwe_ids)
        print(f"找到 {len(related_cves)} 个相关CVE:")
        for cve in related_cves[:3]:
            print(
                f"- {cve['cve_id']}: CVSS {cve['cvss_score']}, {cve['description'][:100]}...")

        # 4. 产品关键词搜索
        print(f"\n[阶段3] 产品关键词搜索 - 关键词: {product_keywords}")
        keyword_results = self.search_cve_by_keywords(product_keywords)
        print(f"找到 {len(keyword_results)} 个匹配:")
        for cve in keyword_results[:3]:
            print(
                f"- {cve['cve_id']}: 分数{cve['keyword_score']:.2f}, {cve['description'][:100]}...")

        # 5. 语义搜索
        print(f"\n[阶段4] 语义搜索 - 查询: '{cve_keywords}'")
        # 首先尝试在关键词结果中搜索
        if keyword_results:
            semantic_search_scope = [cve["cve_id"] for cve in keyword_results]
            semantic_results = self.search_cve_by_semantics(cve_keywords,
                                                            semantic_search_scope)
            print(
                f"在关键词结果中搜索，找到 {len(semantic_results)} 个语义匹配:")
        else:
            # 如果没有关键词结果，则全局搜索
            semantic_results = self.search_cve_by_semantics(cve_keywords)
            print(f"全局语义搜索，找到 {len(semantic_results)} 个语义匹配:")

        for cve in semantic_results[:3]:
            print(
                f"- {cve['cve_id']}: 相似度{cve['semantic_score']:.2f}, {cve['description'][:100]}...")

        # 6. 结果合并
        print("\n[阶段5] 结果合并与排序")
        combined = self._combine_and_rank_results(related_cves, keyword_results,
                                                  semantic_results)
        print(f"生成 {len(combined)} 个最终结果 (前5个):")
        for i, cve in enumerate(combined[:5], 1):
            print(f"   类型: {cve['match_types_str']}")
            print(f"   CVSS: {cve['cvss_score']}")
            print(f"   发布日期: {cve['published_date']}")
            print(f"   描述: {cve['description'][:150]}...\n")

        # 7. 准备LLM输入
        print("[阶段6] 准备LLM输入预览")
        llm_result = self._generate_llm_input(
            product_keywords,
            [cwe_keywords, cve_keywords],  # 作为流量特征
            combined[:5]  # 前5条结果
        )
...
def main():
        ...
        try:
            # 运行详细测试
            print("🧪 运行详细测试流程...")
            results = analyzer.test_search_with_verbose_output()

            print(f"\n✅ 测试完成！共找到 {len(results)} 个候选CVE")

            # 也可以运行自定义测试
            # print("\n🧪 运行自定义测试...")
            # custom_results = analyzer.test_search_with_verbose_output(
            #     product_keywords=['CustomDevice', 'CustomModel'],
            #     cwe_keywords='Buffer Overflow',
            #     cve_keywords='Remote Code Execution in IoT'
            # )
